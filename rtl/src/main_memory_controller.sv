...
module main_memory_controller (
    ...
    assign iomem_wstrb = timer_iomem_valid ? 4'b0 : ((switch == `DATA) ? l1v_iomem_wstrb : 4'b0);

    assign iomem_wdata = l1v_iomem_wdata;

    assign iomem_valid = timer_iomem_valid ? 1'b1 : ((switch == `INSTRUCTION) ? l1b_iomem_valid : l1v_iomem_valid);

    assign iomem_addr  = timer_iomem_valid ? timer_iomem_addr : ((switch == `INSTRUCTION) ? {8'h40,5'b0,l1b_iomem_addr,2'b0}   : {8'h40,5'b0,l1v_iomem_addr,2'b0} ) ;

    assign l1b_iomem_rdata   = iomem_rdata;
    assign l1v_iomem_rdata   = iomem_rdata;
    assign timer_iomem_rdata = iomem_rdata;

    assign l1b_iomem_ready = timer_iomem_valid ? 1'b0 : ((switch == `INSTRUCTION) ? iomem_ready : 1'b0);
    assign l1v_iomem_ready = timer_iomem_valid ? 1'b0 : ((switch == `DATA)   ? iomem_ready : 1'b0);
