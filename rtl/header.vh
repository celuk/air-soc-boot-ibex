`define COREV_PULP 1'b0
`define COREV_CLUSTER 1'b0
`define FPU 1'b0
`define FPU_ADDMUL_LAT 1'b0
`define FPU_OTHERS_LAT 1'b0
`define ZFINX 1'b0
`define NUM_MHPMCOUNTERS 1'b1  

`define BOOT_ADDR 32'h00000080 //32'h00000180
`define MTVEC_ADDR 32'h0
`define DM_HALT_ADDR 32'h00000000 //32'h1A110800

`define PULP_CLOCK_EN 1'b0
`define SCAN_CG_EN 1'b0

`define HART_ID 32'h0
`define DM_EXCEPTION_ADDR 32'h0


`define MEM_BASE_ADDR 32'h0000_0000
`define MEM_END_ADDR 32'h0008_0000
`define MEM_RANGE (`MEM_END_ADDR - `MEM_BASE_ADDR)

`define PERIPH_BASE_ADDR 32'h2000_0000
`define PERIPH_END_ADDR 32'h2007_0000
`define PERIPH_RANGE (`PERIPH_END_ADDR - `PERIPH_BASE_ADDR)


/*
PERIPHERALS
0x20000000 = UART  = 00100000000000000000000000000000
0x20010000 = QSPI  = 00100000000000010000000000000000
0x20020000 = I2C   = 00100000000000100000000000000000
0x20030000 = GPIO  = 00100000000000110000000000000000
0x20040000 = USB   = 00100000000001000000000000000000
0x20050000 = TIMER = 00100000000001010000000000000000
0x20060000 = JTAG  = 00100000000001100000000000000000
*/
`define UART_BASE_ADDR 32'h2000_0000
`define QSPI_BASE_ADDR 32'h2001_0000
`define I2C_BASE_ADDR 32'h2002_0000
`define GPIO_BASE_ADDR 32'h2003_0000
`define USB_BASE_ADDR 32'h2004_0000
`define TIMER_BASE_ADDR 32'h2005_0000
`define JTAG_BASE_ADDR 32'h2006_0000
