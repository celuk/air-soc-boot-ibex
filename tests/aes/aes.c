...
// Rcon (Round Constant - from prompt)
static const uint32_t Rcon[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
    0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000
};

// Static buffer for the result (not thread-safe)
...
// Optimized Galois Field multiplications by specific constants for MixColumns
...
// Inverse AES round transformations
...
uint32_t* aes_encrypt(uint32_t block_part0, uint32_t block_part1, uint32_t block_part2, uint32_t block_part3,
    ...
    AddRoundKey(state, &round_keys[0]); // Round 0
...
uint32_t* aes_decrypt(uint32_t block_part0, uint32_t block_part1, uint32_t block_part2, uint32_t block_part3,
    ...
    KeyExpansion(round_keys, initial_key); // Key expansion is the same for encrypt/decrypt
...
int main() {
    init_uart();
    // Example from FIPS-197 Appendix B (AES-128)
    // Plaintext: 3243f6a8885a308d313198a2e0370734
    // Key:       2b7e151628aed2a6abf7158809cf4f3c
    // Ciphertext:3925841d02dc09fbdc118597196a0b32

    uint32_t p0 = 0x3243f6a8; uint32_t p1 = 0x885a308d;
    uint32_t p2 = 0x313198a2; uint32_t p3 = 0xe0370734;

    uint32_t k0 = 0x2b7e1516; uint32_t k1 = 0x28aed2a6;
    uint32_t k2 = 0xabf71588; uint32_t k3 = 0x09cf4f3c;

    tekno_printf("Plaintext: %08x %08x %08x %08x\n", p0, p1, p2, p3);
    tekno_printf("Key:       %08x %08x %08x %08x\n", k0, k1, k2, k3);

    uint32_t* encrypted_block = aes_encrypt(p0, p1, p2, p3, k0, k1, k2, k3);
    tekno_printf("Encrypted: %08x %08x %08x %08x\n",
           encrypted_block[0], encrypted_block[1], encrypted_block[2], encrypted_block[3]);
    
    // Expected: 3925841d 02dc09fb dc118597 196a0b32
    uint32_t c0 = encrypted_block[0]; uint32_t c1 = encrypted_block[1];
    uint32_t c2 = encrypted_block[2]; uint32_t c3 = encrypted_block[3];

    uint32_t* decrypted_block = aes_decrypt(c0, c1, c2, c3, k0, k1, k2, k3);
    tekno_printf("Decrypted: %08x %08x %08x %08x\n",
           decrypted_block[0], decrypted_block[1], decrypted_block[2], decrypted_block[3]);
